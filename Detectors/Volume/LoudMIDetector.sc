LoudMIDetector : MIDetector{
	...
	*new{|win,in=0,tag=0,args=nil|
	...
	initValues {
		...
		this.checkArg(\smask,0.25);
		this.checkArg(\tmask,1);
		name="Loud";
		nchan=1;
		...
		args[\yaxis]=ControlSpec(0,64,\lin,units:'phon');
	}
	...
	loadSynthDef {
		SynthDef(synthname,{|in=0,gate=1,buf,mult=1|
			...
			chain = FFT(LocalBuf(args[\fftsize]), sig);
			loud=Loudness.kr(chain,args[\smask],args[\tmask]);
			RecordBuf.kr(loud*mult,buf);
		}).load(Server.default);
	}
